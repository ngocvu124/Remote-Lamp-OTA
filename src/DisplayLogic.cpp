...
void DisplayLogic::loadBackgroundFromSD() {
    ...
    if (bg_data_buffer) {
        ...
        custom_bg.header.w = 240; custom_bg.header.h = 240;
        custom_bg.header.cf = LV_IMG_CF_TRUE_COLOR; custom_bg.data_size = 115200;
...
void DisplayLogic::updateUI(RemoteState &state) {
    ...
    if (state.currentMenu == MENU_NONE || state.currentMenu == MENU_SET_SLEEP || state.currentMenu == MENU_SET_BACKLIGHT) {
        ...
        int val = state.isTempMode ? state.temperature : state.brightness;
        lv_arc_set_value(objects.arc_value, val);
        lv_label_set_text_fmt(objects.value, "%d%%", val);
        lv_label_set_text(objects.label_value, state.isTempMode ? "COLOR TEMP" : "BRIGHTNESS");
    } 
    else {
        if (lv_scr_act() != objects.menu) lv_scr_load(objects.menu);
        if (state.currentMenu != lastMenuType) {
            if (state.currentMenu == MENU_MAIN) buildMenu(mainMenuItems, 6); 
                ...
                if (state.currentMenu == MENU_USB_MODE) {
                    ...
                    for (int i = 0; i < ota.versionCount && i < 15; i++) items[i] = ota.versions[i].name;
        ...
        for (int i = 0; i < currentMenuCount; i++) {
            ...
            if (i == state.menuIndex) { lv_obj_add_state(menuButtons[i], LV_STATE_CHECKED); lv_obj_scroll_to_view(menuButtons[i], LV_ANIM_ON); }
...
void DisplayLogic::showFileContent(const char* title, const char* content) {
    if (file_overlay) { lv_obj_del(file_overlay); file_overlay = NULL; }
    if (current_file_buffer) { storage.freePSRAMBuffer(current_file_buffer); current_file_buffer = NULL; }
    if (title == NULL && content == NULL) return; 

    if (title != NULL && strcmp(title, "SCROLL_UP") == 0) return; // Logic cuộn bác tự thêm sau
    if (title != NULL && strcmp(title, "SCROLL_DOWN") == 0) return;
    ...
    lv_obj_t * l_title = lv_label_create(file_overlay);
    lv_label_set_text(l_title, title); lv_obj_align(l_title, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_t * l_cont = lv_label_create(file_overlay);
    lv_label_set_long_mode(l_cont, LV_LABEL_LONG_WRAP); lv_obj_set_width(l_cont, 200);
    lv_label_set_text(l_cont, current_file_buffer ? current_file_buffer : "Empty!");
    lv_obj_align(l_cont, LV_ALIGN_TOP_LEFT, 0, 30);
}
...
void DisplayLogic::showProgressPopup(const char* title, const char* msg, int percent) {
    if (!ota_overlay) {
        ...
        lv_obj_set_width(ota_label_content, 200); lv_obj_align(ota_label_content, LV_ALIGN_TOP_LEFT, 0, 30);
...
void DisplayLogic::closeProgressPopup() { if (ota_overlay) { lv_obj_del(ota_overlay); ota_overlay = NULL; } }